...
void runMenu(App* app){
    ...
    //megkérjÿk a felhasználót hogy adjon meg egy nevet
...
int runGame(App* app){
    ...
    while(app->isGame){
        ...
        //minden framen 2/1 pont nehézségtõl fÿggõen
        score+= app->hardmode? 2 : 1 ;
        sprintf(pont_szoveg , "%d" , score);
        //shot timer léptetése ha szÿkséges
        if(shot_timer<=SHOT_TIME) shot_timer++;
        //ha az elõzõ framen menÿ parancsot ad a játékos akkor kilép a menÿbe
        if(app->input.menu == 1){
            app->isGame=false;
            app->isMenu=true;
            SDL_HideWindow(app->gameWindow);
        ...
        while (SDL_PollEvent(&e)){
            switch (e.type) {
                case SDL_WINDOWEVENT:
                    if(e.window.event == SDL_WINDOWEVENT_CLOSE){ //ha bezárjuk az ablakot visszatérÿnk a menÿbe(alt+f4)
                        app->isGame=false;
                        app->isMenu=true;
                        SDL_HideWindow(app->gameWindow);
                        SDL_ShowWindow(app->menuWindow);
                ...
                case SDL_KEYDOWN: //billentyÿleÿtések kezelése
                    keyDown(&app->input , &e.key);
                    break;
                    
                case SDL_KEYUP: //billentyÿfelengedések kezelése
        ...
        //meteor kettéválasztása ha az eltalált meteor nem a legkisebb méretÿ(egybe kerÿl néha 2 meteor de m1)
